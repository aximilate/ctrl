...
Целевой сценарий:
- фронтенд (статический build Flutter) на `ctrlchat.ru`;
- API на `web.ctrlchat.ru` (`/api` + `/admin`).
...
Пример Nginx:
- `server_name ctrlchat.ru` -> `root /var/www/ctrlchat`.
- `server_name web.ctrlchat.ru` -> `proxy_pass http://127.0.0.1:8080`.
